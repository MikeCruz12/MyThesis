<?php
	...
	$queryApprove="SELECT COUNT(status) as total_approve FROM register WHERE status='Disapproved' AND dateConfirmed=CURDATE()";
	...
	?>
		...
		<h1>
			Today Disapproved Registration (<?php echo $approve_count;?>)
		...
		<?php
			$queryApprove_show="SELECT * FROM register LEFT JOIN setspot ON register.setspot_id = setspot.id LEFT JOIN users ON register.user_id = users.id WHERE status='Disapproved' AND dateConfirmed=CURDATE() ORDER BY datenow desc";
