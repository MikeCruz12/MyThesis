<?php
	...
	if($result){
		while($row = mysqli_fetch_array($result)){
			...
			$arrive = date('Y-m-d', strtotime($date."+" . $days." days"."-1 days"));
			$datenow = date("Y-m-d");
			$guide=$row['tourguide'];
			$gname=$row['guide'];
...
<body>
	<div class="container" style="padding-bottom: 20px;margin-top: 10px;">
		...
		<h2 align="center">Approval Form #<?php echo $id; ?></h2>
		...
		<div class="table-responsive">
			...
			<br>
            <form method="POST">
            	<input type="hidden" name="arrival" value="<?php echo $arrive; ?>" required="">
            	<input type="submit" name="send" value="Send" class="btn btn-primary" style="float: right;">
            </form>
            ...
            if (isset($_POST['send'])){
            	$arr=mysqli_real_escape_string($con,$_POST['arrival']);


            	$update = "UPDATE register SET status='Approved', dateConfirmed='$datenow', arriveDate='$arrive' WHERE id='$id'";

				mysqli_query($con, $update);

				$updateHiker = "UPDATE hikerlist SET last='$arr' WHERE register_id='$id'";
