<?php
	...
	$queryApprove="SELECT COUNT(status) as total_approve FROM register WHERE status='Approved' AND YEARWEEK(`dateConfirmed`, 1) = YEARWEEK(CURDATE(), 1)";
		...
		?>
			...
			<h1>
			This Week Approved Registration (<?php echo $approve_count;?>)
			...
			<?php
				$queryApprove_show="SELECT * FROM register LEFT JOIN setspot ON register.setspot_id = setspot.id LEFT JOIN users ON register.user_id = users.id WHERE status='Approved' AND YEARWEEK(`dateConfirmed`, 1) = YEARWEEK(CURDATE(), 1) ORDER BY datenow desc";
